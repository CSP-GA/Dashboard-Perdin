 ...
 * v5.10 - Hybrid AI+Data kota tujuan (KOTA_JSON)
 */
...
function generateNarasiAI(payload) {
  ...
  var topKotaStr = topKota.map(function(k, i) {
    var tujuanSample = (k.tujuanList || []).slice(0, 15).join(' | ');
    return (i + 1) + '. ' + k.kota +
           ' | ' + k.trip + ' trip, ' + k.user + ' user' +
           (tujuanSample ? '\n   Tujuan: ' + tujuanSample : '');
  }).join('\n');
  ...
  var prompt =
    ...
    'PARAGRAF 2 — Pola & Efisiensi Operasional: Dalam satu kalimat yang mengalir, sambungkan rata-rata durasi ' + avgDur + ' hari per perjalanan, lead time pengajuan ' + avgLead + ' hari, dan SLA persetujuan ' + avgSLAJam + ' jam, lalu beri interpretasi singkat soal kecepatan proses administrasi. Lanjutkan dengan SATU kalimat insight efisiensi biaya: tegaskan bahwa lead time pengajuan yang lebih panjang membuka peluang memperoleh tarif tiket dan akomodasi yang lebih rendah, sehingga pengajuan perjalanan lebih dini berpotensi menekan biaya perjalanan dinas. Gunakan angka persis dan JANGAN mengarang angka/persentase penghematan yang tidak ada di data.\n\n' +
